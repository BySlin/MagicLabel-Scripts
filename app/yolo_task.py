...
def export_task_process(framework, output_path, command, msg_queue):
  sys.stdout = QueueWriter("on_export_log", msg_queue)
  sys.stderr = QueueWriter("on_export_log", msg_queue)
  ...
  if framework == "ultralytics":
    ...
    try:
      params.pop("batch")
      imgsz_ = params.pop("imgsz")
      if isinstance(imgsz_, int):
        params["imgsz"] = (imgsz_, imgsz_)
      format_ = params.pop("format")
      params["weights"] = model_path
      params["include"] = [format_]
      files = export.run(**params)
      if os.path.normpath(model_dir) != os.path.normpath(output_path):
        for output_model_path in files:
          shutil.move(output_model_path, output_path)
    except:
      traceback_info = traceback.format_exc()
      print(f"{traceback_info}")
...
def model_predict_task_process(framework, source, command, msg_queue):
  sys.stdout = QueueWriter("on_model_predict_log", msg_queue)
  sys.stderr = QueueWriter("on_model_predict_log", msg_queue)
  ...
  model_path = params.pop("model")
  conf = params.pop("conf")
  iou = params.pop("iou")
  imgsz = params.pop("imgsz")
  model_dir = os.path.dirname(model_path)
  os.chdir(model_dir)

  try:
    from predict import model_predict
    model_predict(task, framework, os.path.basename(model_path), source, conf, iou, imgsz)
  except:
    traceback_info = traceback.format_exc()
    print(f"{traceback_info}")
